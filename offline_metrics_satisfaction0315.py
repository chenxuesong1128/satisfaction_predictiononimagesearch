...
def cmp_top10():
    satisfaction_dict, results_dict = load_results()
    distribution_2d = defaultdict(lambda: defaultdict(lambda: np.zeros((10, 13), dtype=float)))
                ...
                for row in results_dict[task_id][user][query_id]:
                    for result in results_dict[task_id][user][query_id][row]:
                        ...
                        for judgement in judgement_list:
                            ...
                            distribution_2d['All'][judgement][int(row)][int(column_index)] += result[judgement]
                            distribution_2d[query_type_list[query_type_id]][judgement][int(row)][int(column_index)] += int(result[judgement])
    ...
    # np_all = np.zeros((10, 12), dtype=float)
    # for query_type in query_type_list:
    #     for judgement in judgement_list:
    #         for row in range(10):
    #             np_all[distribution_2d[query_type][judgement][row].keys()] = [distribution_2d[query_type][judgement][row]]
    #
